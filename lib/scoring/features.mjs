...
export function extractFeatures(essayText, topic = null) {
  ...
  return {
    ...
    // Effective length: distinct content, credited up to the length a full GRE
    // Issue essay runs and flat beyond.
    //
    // A linear version was tried, to stop 382 words reading as nearly as long
    // as 600. It made short essays rank correctly but cost real agreement
    // everywhere else and reopened the padding hole, because much of the
    // model's accuracy turned out to rest on features this curve balanced.
    // Log is kept, and the specific failure it cannot see (a short, tidy
    // response scoring high) is handled explicitly by the ceiling in
    // ceiling.mjs, which encodes what the corpus cannot teach.
    lengthAdequacy: Math.log1p(Math.min(wordCount * (1 - Math.min(redundancy, 0.85)), TARGET_WORDS)),
    redundancy,

    paragraphCount: Math.min(paras.length, 7),
    ...
    // Organisation
    concessionCount: Math.min(concessionCount, 3),
    rebuttalCount: Math.min(rebuttalCount, 4),
    concedeAndRebutParagraphs: Math.min(concedeAndRebutParagraphs, 2),
