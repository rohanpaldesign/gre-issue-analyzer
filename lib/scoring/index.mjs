...
export function scoreEssay(essayText, topic = null) {
  ...
  const holistic = Math.min(6, Math.max(1, ETS_ANCHOR.slope * raw + ETS_ANCHOR.intercept));
  ...
  return {
    ...
    band: {
      ...
      halfWidth: half,
      // Stated plainly rather than dressed up. See CALIBRATION_META for the
      // measurements behind it.
      basis: `Fitted on ${CALIBRATION_META.trainSize.toLocaleString()} human-scored essays and anchored on ${CALIBRATION_META.etsAnchors} essays ETS scored itself. Mean error against those is ${CALIBRATION_META.etsMae} of a point.`,
    },
