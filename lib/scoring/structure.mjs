...
export function detectConcessionMoves(text, { window = 4 } = {}) {
  ...
  // Deduplicated by sentence text. Restating the same concession does not make
  // it two concessions, and without this a writer padding an essay by repeating
  // their own sentences inflated every structural count.
  const distinct = (items) => {
    const seen = new Set();
    return items.filter((item) => {
      const key = item.text.toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, ' ').trim();
  ...
  return {
    concessions: distinct(confirmedConcessions),
    rebuttals: distinct(rebuttals),
    completeMoves: distinct(completeMoves.map((m) => ({ ...m, text: m.concession.text }))),
